...
final class Settings {
    ...
    public static function refresh_root_app_id(): void {
        global $wpdb;
        // Single targeted lookup invoked only on install/uninstall transitions.
        // No high-level WP API exposes a join across posts + postmeta filtered
        // by both post_type and meta_key/value, and the result is persisted
        // back into the `dsgo_apps_root_app_id` option (which IS cached) — so
        // request-level caching on top of a once-per-mutation read would be
        // wasted overhead.
        ...
        <div class="dsgo-admin dsgo-admin--settings">
            <header class="dsgo-admin__hero">
                <p class="dsgo-admin__eyebrow"><?php
                    /**
                     * Brand prefix on the settings-page eyebrow. Goes through the
