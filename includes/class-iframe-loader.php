...
final class IframeLoader {
    ...
    private static function render_error_page(int $status, string $title, string $body): void {
        ...
        if (!$rendered) {
            ...
            // $lang/$style/$title_html/$body_html/$home_link are all pre-escaped above
            // (esc_attr/esc_html/esc_url) or built from static authored markup, so the
            // concatenated echo is safe.
