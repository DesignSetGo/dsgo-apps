...
Tags: ai, sandbox, iframe, vibe-coding, app
...
1. Open your AI builder of choice and ask it to build the thing — a calculator, a quiz, a product configurator, a custom landing page, an internal dashboard, whatever.
...
* **Iframe** — the app runs inside a sandboxed `<iframe sandbox="allow-scripts">` with an opaque origin. Single-page, not crawlable, but the safest default for code you didn't write yourself (pasted AI artifacts, AI-generated bundles you haven't reviewed, etc.).
...
* **Drop in a bundle (the default)** — zip up the static build, or just upload the raw `.html` from any AI artifact. The plugin handles the rest. This is the path most sites use.
* **Vibe-code in your IDE, deploy from the terminal** — the optional `@designsetgo/cli` package (`npx designsetgo apps deploy`) authenticates as a `manage_options` admin via a one-time application password, packages the current working directory, and pushes it through the same install endpoint. Great for AI coding assistants (and any workflow where the app source lives in a normal project). (Pro lifts the cap so you can iterate freely; Lite users can use the CLI but stay subject to the 1-app cap.)
...
* AI-artifact upload importer — paste an HTML file or a static `.zip` and ship it as a sandboxed app in seconds
...
* CLI (`@designsetgo/cli`) with `init`, `login`, `deploy`, `list` — works with any AI coding assistant out of the box
...
The plugin core does not transmit data to DesignSetGo, and does not phone home for updates, license checks, or analytics. **Apps installed by the admin can, however, make outbound calls to third-party services that the admin has explicitly allowlisted in the manifest:**

* `dsgo.http.fetch` — an HTTP proxy for app-to-external-API calls (Stripe, Notion, Airtable, etc.). The manifest must declare each host, and the admin approves the allowlist at install time. Requests originate from the WordPress server, not from DesignSetGo.
* `dsgo.ai.prompt` and AI-backed abilities — route through the WordPress 7.0 AI Client to whichever Connector you configured at **Settings → Connectors**. Prompts go directly from your site to whichever provider you chose (Anthropic, OpenAI, Google, etc.); you hold the API key and the billing relationship.
...
* CLI (`@designsetgo/cli`) with `init`, `login`, `deploy`, `list` commands; `--from-artifact` flag for pasted AI artifact bundles.
