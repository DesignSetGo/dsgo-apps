...
(function () {
    ...
    function upload(file) {
        ...
        if (installInFlight) {
            showStatus(__('An install is already in progress — finish or cancel it first.', 'dsgo-apps'), 'error');
            return;
        }
    ...
    function previewAndInstall(file) {
        ...
        showStatus(sprintf(__('Validating %s…', 'dsgo-apps'), file.name));
        setProgress(8);
        ...
        xhr.addEventListener('load', function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                setProgress(70);
                showStatus(__('Review permissions and confirm to install.', 'dsgo-apps'));
    ...
    function openInstallConsent(file, preview) {
        ...
        var title = preview.is_update
            ? sprintf(
                ...
                __('Update "%1$s" to v%2$s?', 'dsgo-apps'),
                preview.name || preview.app_id,
                preview.version,
            )
            : sprintf(
                /* translators: 1: app name, 2: app version */
                __('Install "%1$s" v%2$s?', 'dsgo-apps'),
                preview.name || preview.app_id,
                preview.version,
            );
    ...
    function finalizeInstall(file, preview) {
        showStatus(sprintf(__('Installing %s…', 'dsgo-apps'), file.name));
        setProgress(80);
        ...
        xhr.addEventListener('error', function () {
            installInFlight = false;
            closeConsent();
            showStatus(__('Install failed — check your connection and try again.', 'dsgo-apps'), 'error');
            setProgress(0);
        });
        xhr.send(form);
    }
    ...
    function importErrorMessage(code, fallback) {
        switch (code) {
            ...
            case 'lite_cap_reached':
                return __('The free version allows one active app per site. Remove the existing app, or upgrade to Pro for unlimited apps + Riff (the in-admin AI app builder).', 'dsgo-apps');
            default:
