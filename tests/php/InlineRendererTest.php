...
class InlineRendererTest extends WP_UnitTestCase {
    ...
    public function test_route_claims_path_returns_true_for_claimed_static_route(): void {
        $arr = $this->minimal_inline_manifest();
        $arr['mount'] = ['mode' => 'root'];
        $arr['routes'][] = ['path' => '/blog', 'file' => 'blog.html', 'claim' => 'always'];
        $manifest = Manifest::validate($arr);
        $this->assertTrue(InlineRenderer::route_claims_path($manifest, '/blog'));
    }

    public function test_route_claims_path_returns_true_for_claimed_dynamic_route(): void {
        $arr = $this->minimal_inline_manifest();
        $arr['mount'] = ['mode' => 'root'];
        $arr['routes'][] = [
            'path'    => '/blog/:slug',
        ...
        $manifest = Manifest::validate($arr);
        $this->assertTrue(InlineRenderer::route_claims_path($manifest, '/blog/some-post'));
    }

    public function test_route_claims_path_returns_false_for_unclaimed_route(): void {
        $arr = $this->minimal_inline_manifest();
        $arr['mount'] = ['mode' => 'root'];
        $arr['routes'][] = ['path' => '/blog', 'file' => 'blog.html']; // no claim
        $manifest = Manifest::validate($arr);
        $this->assertFalse(InlineRenderer::route_claims_path($manifest, '/blog'));
    }

    public function test_route_claims_path_returns_false_when_no_route_matches(): void {
        $arr = $this->minimal_inline_manifest();
        $arr['mount'] = ['mode' => 'root'];
        $arr['routes'][] = ['path' => '/blog', 'file' => 'blog.html', 'claim' => 'always'];
        $manifest = Manifest::validate($arr);
        // /blog/typo isn't in the routes list at all.
        $this->assertFalse(InlineRenderer::route_claims_path($manifest, '/blog/typo'));
    }
    ...
    public function test_rewrite_bundle_asset_paths_rewrites_existing_files(): void {
        ...
        // Static assets go to the upload URL (nginx-bypass). Per-file mtime
        // cache-bust query string is appended; assertion accepts it optionally.
        $upload_path_re = preg_quote($upload_path, '#');
        $this->assertMatchesRegularExpression('#src="' . $upload_path_re . '/_astro/foo\.js(?:\?v=\d+)?"#', $out);
        $this->assertMatchesRegularExpression('#href="' . $upload_path_re . '/_astro/foo\.js(?:\?v=\d+)?"#', $out);
    ...
    public function test_root_mount_rewrites_bundle_assets_to_upload_url(): void {
        ...
        $upload_path_re = preg_quote(rtrim($upload_path, '/'), '#');
        $this->assertMatchesRegularExpression('#src="' . $upload_path_re . '/_astro/foo\.js(?:\?v=\d+)?"#', $out);
        $this->assertMatchesRegularExpression('#href="' . $upload_path_re . '/favicon\.svg(?:\?v=\d+)?"#', $out);
    ...
    public function test_render_publisher_host_injects_iframe_bridge_client_with_nonce(): void {
        ...
        // Bridge-client URL is cache-busted with ?ver=<mtime>; allow the
        // query string optionally so the assertion survives the cache-bust.
        $expected_src_re = preg_quote(esc_url(plugins_url('assets/bridge-client.js', DSGO_APPS_FILE)), '#');
        $this->assertMatchesRegularExpression('#<script src="' . $expected_src_re . '(?:\?ver=\d+)?"#', $output);
        $this->assertStringContainsString('nonce="NONCE-FIXED"', $output);
    }
    ...
    public function test_render_publisher_host_rewrites_bundle_asset_paths(): void {
        ...
        $upload_path = rtrim((string) wp_parse_url(Bundle::url_for($manifest->id), PHP_URL_PATH), '/');
        $upload_path_re = preg_quote($upload_path, '#');
        $this->assertMatchesRegularExpression('#src="' . $upload_path_re . '/_astro/foo\.js(?:\?v=\d+)?"#', $output);
    }
